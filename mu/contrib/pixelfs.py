...
def find_pixelkit():
    ...
    for port in ports:
        if "VID:PID=0403:6015" in port[2].upper():
...
def get_serial():
    """
    Detect if a Pixel Kit is connected and return a serial object to talk to
    it.
    """
    port, serial_number = find_pixelkit()
    if port is None:
        raise IOError('Could not find Pixel Kit.')
