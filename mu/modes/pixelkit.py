...
class PixelKitMode(MicroPythonMode):
    ...
    description = _("Write MicroPython for the Kano Pixel Kit.")
    icon = 'microbit'
    ...
    def copy_main(self):
        ...
        if self.python_script.strip():
            ...
            serial = pixelfs.get_serial()
            out, err = pixelfs.execute(commands, serial)
            logger.info((out, err))
            if err:
                raise IOError(pixelfs.clean_error(err))
            # Reset the device.
    ...
    def toggle_repl(self, event):
        ...
        if self.fs is None:
            ...
            if self.repl:
                self.set_buttons(flash=False, files=False, run=True, stop=True,)
            elif not (self.repl or self.plotter):
                self.set_buttons(flash=True, files=True, run=True, stop=True,)
        else:
        ...
        if (self.repl or self.plotter):
            ...
            if self.fs is None:
                ...
                if self.fs:
                    ...
                    self.set_buttons(flash=False, repl=False, run=False, stop=False, plotter=False)
            else:
                self.remove_fs()
                logger.info('Toggle filesystem off.')
                self.set_buttons(flash=True, repl=True, run=True, stop=True, plotter=True)
